...
results_dir = jp(cwd, 'results', sub_folder_name, 'scenario', 'WFA')  # Results files directory
...
reference_model_file = jp(cwd,
                          'ref',
                          'paper',
                          'forwardscenariostartmodel.dat')

# %% 12 File for reference model (model weights)

reference_weights_file = jp(cwd,
                            'results',
                            'paper',
                            'scenario', 'WFB', 'STEPII', 'interpreted_model', 'config', 'ref_weights.dat')
...
smooth_file = \
    jp(cwd, 'results', 'paper', 'scenario', 'WFB', 'STEPI', 'rho14.txt')
smooth_wfa = crc.res2mod(smooth_file, processing_function=my_func)
...
cut = np.log10(4500)
rest[rest > cut] = cut
res_levels = 10 ** np.linspace(min(rest), max(rest), 7)  # Define a linear space for the color map
rtp = 10 ** np.copy(rest)
...
if not dc:
    ...
    model_map(polygons=blocks, vals=ipt, log=0, cbpos=0.4, levels=ip_levels, folder=result_folder,
