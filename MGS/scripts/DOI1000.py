"""
Compute the DOI with resistivity value 1000
"""
...
results_dir = jp(cwd, 'results', sub_folder_name, 'field_data', 'DOI1000')  # Results files directory
...
with open(reference_model_file, 'w') as smf:
    smf.write(str(nelem) + '\n')
    [smf.write(str(10 ** val) + '\t' + '0' + '\t' + str(val) + '\t' + '0' + '\n')
...
smooth_file = \
    jp(cwd, 'results', 'paper', 'field_data', 'WFB', 'STEPI', 'rho27.txt')
smooth_wfa = crc.res2mod(smooth_file, processing_function=my_func)
