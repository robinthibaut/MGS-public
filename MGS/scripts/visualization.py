...
results_dir = jp(cwd, 'results', sub_folder_name, 'FM1B')  # Results files directory
...
try:
    ...
    model_map(polygons=blocks, vals=rtp, log=1, cbpos=0.35, levels=res_levels, folder=result_folder, figname='res')
    plt.show()
except ValueError:
    pass
...
model_map(polygons=blocks, vals=ss, log=0, levels=s_levels,
          stepy=8, contour=-3, aspect=1.5,
          cbpos=0.2, folder=result_folder, figname='sens')
plt.show()
...
plt.savefig('pseudosection.png', bbox_inches='tight', transparent=False, dpi=300)
plt.show()
